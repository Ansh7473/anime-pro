...
// Helper to use Vite proxy in dev, direct in prod (Capacitor/Native where CORS doesn't apply)
const BASE_URL = import.meta.env.DEV ? '/animelok-api' : 'https://animelok.xyz/api/anime';

export async function searchAnimelok(query: string) {
  try {
    // Try suggestions first
    const sUrl = `${BASE_URL}/search-suggestions?q=${encodeURIComponent(query)}`;
...
export async function getAnimelokMetadata(slug: string) {
  ...
  try {
    while (page < 10) {
      const url = `${BASE_URL}/${slug}/episodes-range?page=${page}&pageSize=${pageSize}`;
      const res = await fetch(url, { headers: ANIMELOK_HEADERS });
      if (!res.ok) break;
...
export async function getAnimelokSources(slug: string, ep: number | string) {
  try {
    const url = `${BASE_URL}/${slug}/episodes/${ep}`;
    const res = await fetch(url, { headers: ANIMELOK_HEADERS });
    if (res.ok) {
      const data = (await res.json()) as any;
