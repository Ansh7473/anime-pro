 ...
 */
export async function searchAnimeHindiDubbedWP(
  query: string,
): Promise<AnimeHindiDubbedSearchResult[]> {
  try {
    ...
    if (!response.ok) {
      console.error(
        "[AnimeHindiDubbed-WP] Search failed:",
        response.status,
        response.statusText,
      );
      return [];
 ...
 */
function extractServerVideos(
  html: string,
): Record<string, AnimeHindiDubbedEpisode[]> | null {
  try {
    ...
    console.log(
      "[AnimeHindiDubbed-WP] Extracted serverVideos string length:",
      jsonStr.length,
    );
    ...
    console.log(
      "[AnimeHindiDubbed-WP] Found servers:",
      Object.keys(serverVideos),
    );
    return serverVideos;
  } catch (error) {
    console.error(
      "[AnimeHindiDubbed-WP] Error extracting serverVideos:",
      error,
    );
    return null;
  }
 ...
 */
export async function getAnimeHindiDubbedInfoWP(
  postId: number,
): Promise<AnimeHindiDubbedInfo | null> {
  try {
    ...
    if (!response.ok) {
      console.error(
        "[AnimeHindiDubbed-WP] Failed to get post:",
        response.status,
        response.statusText,
      );
      return null;
...
export async function getAnimeHindiDubbedSourcesWP(
  ...
  try {
    console.log(
      "[AnimeHindiDubbed-WP] Getting sources for post ID:",
      postId,
      "episode:",
      episodeName,
    );
    ...
    const matchingEpisodes = info.episodes.filter((ep) => {
      ...
      return (
        epNameLower.includes(targetLower) ||
...
export async function getAnimeHindiDubbedAllSourcesWP(
  ...
  try {
    console.log(
      "[AnimeHindiDubbed-WP] Getting all sources for post ID:",
      postId,
      "episode:",
      episodeNumber,
    );
    ...
    for (const server of servers) {
      ...
      const matchingEpisodes = episodes.filter((ep) => {
        ...
        return (
          epNameLower.includes(`episode ${epNumStr}`) ||
 ...
 */
async function extractServerVideosFromPost(
  postId: number,
  ...
  try {
    ...
    console.error(
      "[AnimeHindiDubbed-WP] Error extracting serverVideos from post:",
      error,
    );
    return null;
  }
