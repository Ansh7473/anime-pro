...
function generateSlugCandidates(query) {
    const candidates = [];
    ...
    try {
        ...
        if (animeId) {
            try {
                ...
                const ajaxRes = await retryWithBackoff(async () => {
                ...
                if (ajaxRes.ok) {
                    ...
                    if (ajaxData.success && ajaxData.data?.episodes) {
                        episodes = ajaxData.data.episodes.map((ep) => ({
        ...
        return {
            ...
            synopsis: $(".wp-content p, .synopsis, .description")
                ...
                .text()
                .trim(),
            image: $(".poster img, img").first().attr("src") ||
            ...
            if (items.length > 0) {
                items.each((_, el) => {
                    ...
                    if (embedData) {
                        ...
                        const isDub = playerId.toLowerCase().includes("dub") ||
                        ...
                        const isDub = srcLower.includes("dub") ||
                ...
                if (sources.length > 0) {
                    console.log("[DesiDub] Found", sources.length, "sources with selector:", selector);
                    break;
                }
            }
        }
...
//# sourceMappingURL=desidub-v2.js.map