 ...
 */
export async function searchAnimeHindiDubbedWP(query) {
    try {
 ...
 */
function extractServerVideos(html) {
    try {
 ...
 */
export async function getAnimeHindiDubbedInfoWP(postId) {
    try {
 ...
 */
export async function getAnimeHindiDubbedSourcesWP(postId, episodeName) {
    try {
        ...
        const sources = matchingEpisodes
            .filter((ep) => !ep.url.includes("listeamed.net"))
            .map((ep) => {
            // Determine server from URL
            let server = "unknown";
            if (ep.url.includes("bysewihe.com"))
                server = "bysewihe";
            else if (ep.url.includes("short.icu"))
                server = "servabyss";
            else if (ep.url.includes("listeamed.net"))
                server = "vidgroud";
            else if (ep.url.includes("filemoon"))
                server = "filemoon";
            else if (ep.url.includes("mixdrop"))
                server = "mixdrop";
            return {
                url: ep.url,
                type: "iframe",
 ...
 */
export async function getAnimeHindiDubbedAllSourcesWP(postId, episodeNumber) {
    try {
        ...
        for (const server of servers) {
            ...
            for (const ep of matchingEpisodes) {
                if (ep.url.includes("listeamed.net"))
                    continue;
 ...
 */
async function extractServerVideosFromPost(postId) {
    try {
        const url = `${API_BASE}/${postId}`;
        const response = await fetch(url, {
            headers: {
...
//# sourceMappingURL=animehindidubbed-wp.js.map