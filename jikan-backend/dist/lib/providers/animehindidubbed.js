...
export async function searchAnimeHindiDubbed(query) {
    try {
...
export async function getAnimeHindiDubbedInfo(id) {
    try {
        ...
        const scriptContent = $("script")
            ...
            .html() || "";
        let episodes = [];
        let videoData = { bysewihe: [], servabyss: [], vidgroud: [] };
        ...
        if (match && match.index !== undefined) {
            try {
                ...
                if (videoData.bysewihe && Array.isArray(videoData.bysewihe)) {
                    episodes = videoData.bysewihe.map((ep) => ({
                        name: ep.name || "",
                        url: ep.url || "",
                        server: "bysewihe",
                    }));
                ...
                if (videoData.servabyss && Array.isArray(videoData.servabyss)) {
                    videoData.servabyss.forEach((ep) => {
                        if (!episodes.find((e) => e.name === ep.name)) {
                            episodes.push({
                                name: ep.name || "",
                                url: ep.url || "",
                                server: "servabyss",
...
export async function getAnimeHindiDubbedSources(id, episodeName) {
    try {
        ...
        let episode = info.episodes.find((ep) => ep.name.toLowerCase() === episodeName.toLowerCase() ||
        ...
        if (!episode) {
            ...
            if (epPattern) {
                ...
                const matchedEp = info.episodes.find((ep) => ep.name.toLowerCase().includes(pattern.toLowerCase()));
                if (matchedEp) {
                    episode = matchedEp;
                }
            }
        }
        if (!episode || episode.url.includes("listeamed.net"))
            return [];
        // Add source based on server type
        const isEmbed = episode.server !== "bysewihe";
        sources.push({
            name: episode.server === "bysewihe"
                ? "Bysewihe"
                : episode.server === "servabyss"
                    ? "Abysscdn"
                    : "Listeamed",
            url: episode.url,
            category: "hindi",
...
export async function getAnimeHindiDubbedAllSources(id) {
    try {
        const info = await getAnimeHindiDubbedInfo(id);
        if (!info || !info.episodes)
            return [];
        ...
        for (const episode of info.episodes) {
            if (episode.url.includes("listeamed.net"))
                continue;
            const isEmbed = episode.server !== "bysewihe";
            sources.push({
                name: episode.server === "bysewihe"
...
//# sourceMappingURL=animehindidubbed.js.map