...
const jikanRateLimit = async (_c, next) => {
    const now = Date.now();
...
router.get("/schedule", jikanRateLimit, async (c) => {
    ...
    try {
        ...
        if (kids)
            url.searchParams.append("kids", kids);
        if (sfw)
            url.searchParams.append("sfw", sfw);
        const response = await fetch(url.toString());
        if (!response.ok) {
            throw new HTTPException(response.status, {
                message: `Jikan API error: ${response.statusText}`,
...
router.get("/seasons/now", jikanRateLimit, async (c) => {
    ...
    try {
        ...
        if (unapproved)
            url.searchParams.append("unapproved", "true");
        if (continuing)
            url.searchParams.append("continuing", "true");
        const response = await fetch(url.toString());
        if (!response.ok) {
            throw new HTTPException(response.status, {
                message: `Jikan API error: ${response.statusText}`,
